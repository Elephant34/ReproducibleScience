...
To visually see if their is any interaction between culmen depth and body mass it is important to firstly make an exploratory plot before attempting to test for significance.
```{r ExploratoryPlot}
...
```
### Results & Discussion
The plot shows the interaction between body mass and culmen depth for just Adelie penguins as in the ANOVA test above.
```{r PlottingResults}
...
The final results plot shows the same model method applied to all the penguin species.
